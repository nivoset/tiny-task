...
export class Flow<SharedData extends Record<string, unknown> = Record<string, unknown>> {
  ...
  private readonly logger = createFlowLogger();
  ...
  public async execute(shared: SharedData): Promise<SharedData> {
    ...
    while (current) {
      this.logger.debug('Executing current task', { taskName: current.name });
      const nextTaskName = await current.run(shared);
      
      if (nextTaskName === 'end') {
        this.logger.info('Flow execution ended');
        break;
      }
      
      const nextTask = current.successors[nextTaskName];
      if (!nextTask) {
        this.logger.warn('No successor found for action', { 
          action: nextTaskName, 
      ...
      this.logger.debug('Moving to next task', { 
        from: current.name, 
        to: nextTask.name, 
        action: nextTaskName 
      });
   ...
   */
  public reset(): void {
    this.logger.debug('Resetting flow to start task');
  ...
  public setCurrentTask(task: ITask<SharedData>): void {
    this.logger.debug('Setting current task', { taskName: task.name });
...
export function chainTasks<SharedData extends Record<string, unknown>>(
  ...
  if (tasks.length === 0) {
    throw new Error('At least one task is required to create a flow');
  }
  ...
  for (let i = 0; i < tasks.length - 1; i++) {
    ...
    if (currentTask && nextTask) {
      currentTask.connect(nextTask, 'default');
    }
  }
